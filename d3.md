```js
// https://d3js.org v5.7.0 Copyright 2018 Mike Bostock(官网和版权信息)
(
...
- ascending函数
  ...
  - 疑问：顾名思义，这是函数用来进行上升排序，但是引人发问的之处在于，作者为什么不用如下代码呢？

    `return a < b ? -1 : a > b ? 1 : 0;`

  - 解答：字符串、undefined、NaN无论怎样与数字比较，返回值都是false。这样做是为了排除不同类别之间的比较

  - 注意：**当发生不同类别之间的比较时会返回NaN，这是作者使用的一个技巧**。后面的代码可以很容易地通过isNaN()来判断是否发生了不同类别比较的情形，为后面代码做了非常好的铺垫。
```js
...
- bisector函数
  ...
  - 函数名含义及函数用途
      ...
      bi:单词前缀，表示“二”（bicycle自行车，单词“bi-cycle”源自“bi-circle”，直译过来就是两个轮子）；sect：单词词根，表示“分隔”；所以这个函数名的含义就是“二分”；
      
      我想，有些数学基础的同学应该会自然而然地联想到“二分法”
      
      值得注意的是，这个函数返回的并不是使用二分法求到的值，而是一个包含`left`和`right`两个方法的对象，想要求值，我们还需要再次去调用该对象的两个方法。
  
  - 参数含义、类型及用途

      compare：单词词义“比较”
  
      由下面的代码`compare.length`（数组及类数组对象操作)及`compare(a[mid],x)`（函数调用）可知，compare参数的类型应该是**元素为函数、数组长度为1的数组**或**函数**
  
```js
function bisector(compare) {
  // 如果compare是长度为1的数组，则调用ascendingComparator函数重新为compare赋值，并保证compare为函数类型
  ...
  return {
    /*
    	...
    	@x：；
    	@lo：范围最小值，默认是0；
    	@hi：范围最大值，默认是参数a的数组长度
    */
    left: function(a, x, lo, hi) {
      if (lo == null) lo = 0;
      if (hi == null) hi = a.length;
      while (lo < hi) {
        // 注意：“+”号的优先级高于“>>>”，所以下面这句代码的含义是：最大值加最小值的和除以2并向下取整
        var mid = lo + hi >>> 1;
...
- pair函数及pairs函数
  ...
  pair单词含义是配对，用途就是把两个变量转换成一个二元数组
```js
...
- 正交
  ...
  - 依次把第一个数组中的元素依次与第二个数组中的元素配对，并将得到的所有数组组成一个新数组。

  - 新数组的长度 = 第一个数组的长度 * 第二个数组的长度。
  - 参数reduce的默认值是pair函数
  - ~~有点类似于矩阵的乘法运算，不同之处在于，矩阵乘法运算要求第一个矩阵的列数等于第二个矩阵的行数。~~
...
- number函数
  ...
  尝试将参数x转换为数值类型，如果x是null或其他非数值类型则会返回NaN
```js
function number(x) {
  return x === null ? NaN : +x;
}
...
- 样本方差
  ...
  注意这里的措辞是“样本”方差，样本方差的计算公式和方差的计算公式好像不一样。$ S_{n-1} $
...
- 标准差
  ...
  如果v是0的话就返回0，这样就不用再去用`Math.sqrt(v) `算一遍了，这是一种简化计算的方法。
```js
...
- sequence函数
  ...
  功能：在给出起止数据及步长的情况下，自动生成一个序列（数组类型，也可以理解成是区间）。可以接受字符串类型的数字。

  举个例子：

  ​	`sequence(0, 5, 1)			=> [0, 1, 2, 3, 4]`

  ​	`sequence(0, 5.5, 1)		=> [0, 1, 2, 3, 4, 5]`

```js
function sequence(start, stop, step) {
  //+表示将start转换为数值类型，stop、step同理
  start = +start,
  stop = +stop, 
  // 如果sequence函数只有一个参数，默认这个参数是stop，此时start=0，step=1；
  // 如果sequence函数有两个参数，默认这两个参数分别是start、stop，此时step=1；
  step = (n = arguments.length) < 2 ? (stop = start, start = 0, 1) : 
  		n < 3 ? 1 : 
         +step;
  ...
  var i = -1,
      ...
      n = Math.max(0, 
          ...
          ) | 0,	// 保险数0，防止因start、stop及step中有非数值数据而报错
      range = new Array(n);
...
- histogram函数
  ...
  说实话，作者的写法很像Python中的**可执行对象**

```js
function histogram() {	// 这里的histogram只是一层封装
  var value = identity,
      domain = extent,
      threshold = thresholdSturges;

  function histogram(data) {	// 这里的histogram才是真正用来制图的，也是外层histogram的返回值
    var i,
        n = data.length,
        x,
        values = new Array(n);
...
- permute函数
  ...
  功能：把array数组中的被indexes数组选中的元素按照indexes数组中元素的顺序排列为一个新的数组

  示例：`permute('hello world!', [8,2,7,1])	=>["r", "l", "o", "e"]`
  ...
  function axis(context) {
    ...
    path
        .attr("d", 
            ...
            (
                tickSizeOuter ? 
                 "M" + k * tickSizeOuter + "," + range0 + 
        	     "H0.5V" + range1 + 
        		"H" + k * tickSizeOuter :
        		"M0.5," + range0 + "V" + range1
            ): (
                tickSizeOuter ? 
                "M" + range0 + "," + k * tickSizeOuter + 
        		"V0.5H" + range1 + 
        		"V" + k * tickSizeOuter : 
...
- get函数
  ...
  从对象数组中找出特定name的对象并获取他的value属性值
